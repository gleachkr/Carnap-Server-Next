/**
 * Seed the Gentzen classical-sequent-calculus (LK) tree-proof demo onto a running
 * LOCAL dev server (`bun run dev`). Logs in as the local site_admin via the
 * passwordless local flow (the confirm link is returned in the page, no email),
 * then creates a course, authors the demo lesson as a content revision, and
 * publishes it as a practice assignment. Prints the URLs to open.
 *
 *   bun run dev            # in another terminal
 *   bun run scripts/seed-gentzen-demo.ts
 *
...
const course = await postJson("/courses", {
  timezone: "UTC",
  title: "Logic demo — Gentzen sequent calculus",
});
const courseId = pick(course, "course", "id");
console.log(`Course ${courseId}`);

const item = await postJson("/content", {
  title: "Gentzen LK — sequent-calculus tree proofs",
});
...
const assignment = await postJson(`/courses/${courseId}/assignments`, {
  ...
  description: "Tree-shaped proofs in Gentzen's classical sequent calculus (LK).",
  title: "Sequent calculus practice",
});
