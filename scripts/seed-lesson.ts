/**
 * Seed *any* carnap-markdown file onto a running LOCAL dev server (`bun run dev`).
 * The generic sibling of `seed-forallx-demo.ts` / `seed-gentzen-demo.ts`, which
 * each hard-wire one lesson from `tests/helpers`: this one takes a path, so any
 * hand-written lesson can be put in front of the real editors — the quickest way
 * to check an authoring change end to end — without a bespoke script. Logs in as
 * the local site_admin via the passwordless local flow (the confirm link is
 * returned in the page, no email), creates a course, authors the file as a
 * content revision, and publishes it as a practice assignment.
 *
 *   bun run dev                                        # in another terminal
 *   bun run scripts/seed-lesson.ts --file=path/to/lesson.md
 *
 * Flags: --file=PATH (required), --title=TEXT, --base=URL
 * (default http://localhost:8787), --email=ADDR.
 */
...
const item = await postJson("/content", { title: TITLE });
