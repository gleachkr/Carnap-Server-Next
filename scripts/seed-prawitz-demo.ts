/**
 * Seed the Prawitz-tree demo (forallx natural deduction, drawn as trees) onto
 * a running LOCAL dev server (`bun run dev`). Logs in as the local site_admin
 * via the passwordless local flow (the confirm link is returned in the page,
 * no email), then creates a course, authors the demo lesson as a content
 * revision, and publishes it as a practice assignment. Prints the URLs to
 * open.
 *
 *   bun run dev            # in another terminal
 *   bun run scripts/seed-prawitz-demo.ts
 *
